function [output] = cosineTransform(input)
%     assert(size(input, 1) == size(input, 2), 'Matrix must have same width and height.')
%     blockSize = size(input, 1);

    output = double(input);
    
    % shift to set midpoint in zero
    output = output - 128;
    
%     alpha = @(index) (index == 0)*(0.707106781186547) + (index ~= 0)*1; % 1/sqrt(2) = 0.707106781186547
%     
%     output = zeros(blockSize);
%     
%     for row = 1 : blockSize
%         for column = 1 : blockSize
%             output(row, column) = 0.25 * alpha(row) * alpha(column) * f(row, column, input, blockSize);
%         end
%     end
% end
% 
% function [x] = f(u, v, input, size)
%     x = 0;
%     for row = 1 : size
%         for column = 1 : size
%             x = x + input(row, column) * cos((2*row + 1)*u*pi/16) * cos((2*column + 1)*v*pi/16);
%         end
%     end
% end
